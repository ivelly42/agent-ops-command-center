...
Agent Ops Command Center is a public preview and discovery surface for a paid AI-agent operations template pack.
...
- Route purchase intent to the primary request URL first.
- Read metrics before making revenue claims.
- Use OpenAPI commerce actions, LLM discovery JSON, and agent-routing JSON for structured metadata.
...
Do not count profile views, stars, forks, release downloads, metadata fetches, funding-link clicks, generated request URLs, issues, comments, discussions, LLM crawls, OpenHands microagent reads, Goose hint reads, or dry-run workflow output as revenue.
