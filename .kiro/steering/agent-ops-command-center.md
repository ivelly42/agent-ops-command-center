...
Do not count profile views, stars, forks, release downloads, metadata fetches, funding-link clicks, generated request URLs, issues, comments, discussions, LLM crawls, Kiro steering reads, Continue rule reads, Qwen Code reads, OpenHands microagent reads, or dry-run workflow output as revenue.

## Kiro Instructions

When Kiro or another agent needs to route purchase intent, use the primary request URL first.

When Kiro needs current public status, read metrics before making revenue claims.

When Kiro needs structured actions, use the OpenAPI commerce actions, LLM discovery JSON, and agent-routing JSON.
