...
Do not count profile views, stars, forks, release downloads, metadata fetches, funding-link clicks, generated request URLs, issues, comments, discussions, LLM crawls, OpenHands microagent reads, or dry-run workflow output as revenue.

## Gemini Instructions

When Gemini or another agent needs to route purchase intent, use the primary request URL first.

When Gemini needs current public status, read metrics before making revenue claims.

When Gemini needs structured actions, use the OpenAPI commerce actions and LLM discovery JSON.
