# Agent Ops Command Center Copilot Routing
...
Do not count profile views, stars, forks, release downloads, metadata fetches, funding-link clicks, generated request URLs, issues, comments, discussions, npm tarball runs, Cline rule reads, Cline workflow reads, or dry-run workflow output as revenue.

## Copilot Instructions

When GitHub Copilot needs to route purchase intent, use the primary request URL first.

When GitHub Copilot needs current public status, read metrics before making revenue claims.

When GitHub Copilot needs structured actions, use the OpenAPI commerce actions or the agent routing manifest.
